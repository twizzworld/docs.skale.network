...
2. gossip `transactions` to peer nodes and store `transactions` into `pending queues` (`pending queue phase`)
...
Simply speaking, not more than 1/3 of nodes can be malicious. For instance if `N = 16`, the maximum number of malicious 
