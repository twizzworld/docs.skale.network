...
As any Ethereum-compatible chain, a SKALE chain includes `blockchain`, which is a chain of `transactions` ordered into `committed blocks`, and a computing machine denoted as `EVM`.  The set of variables stored in `EVM` is denoted as `EVM state`. 

[mermaid]
....
flowchart LR
    b0[Tx1 processed]---b1[Tx2 processed]---b2[Tx3 processed]---|EVM Processing|b3[Tx4 unprocessed]   
....

`EVM` processes `committed blocks` one `transaction` at a time. For each `transaction`, `EVM` runs instructions (`bytecode`) specified by the `transaction` and changes `EVM state`.

=== Architecture overview

The purpose of a SKALE chain is to order `transactions` into `blocks` and then process them by `EVM`.

SKALE chain is composed of a fixed set of `N` network `nodes`. `Nodes` process user `transactions` according to the following phases
...
. sign a statement specifying the winning proposal (`block signature share`) and broadcast it to other `nodes`. Wait until receipt of $2/3N$ of `block signature shares`. Merge `block signature shares`  into `block signature`. (_block signature phase_)
. commit the `winning proposal` if `node` has it, otherwise download it from other `nodes` and commit it. The `winning proposal` together with `block signature` becomes a`committed block` (_block finalization phase_)
...
`SKALE consensus` is _provably secure_. This means one can prove two qualities of the `blockchain`
...
some `nodes` may, at a given moment, have less `committed blocks` than others. Therefore, _the consistency is eventual_.
...
A `malicious node` will attempt to break the consistency and liveliness of the network by sending `malicious messages`, or by not sending 
...
Algorithms used by `SKALE consensus' make assumptions about _the properties of the underlying network_.
...
The asynchronous model described above is _similar to the model assumed by Bitcoin and Ethereum blockchains_. It reflects _the state of modern Internet_, where temporary network splits and interruptions are normal, but always resolve eventually.

Since in real life Internet sometimes drops messages without delivering them, _the eventual delivery guarantee is achieved in practice by retransmissions_. A `sending node` will make _multiple attempts to transfer_  `message` to a`receiving node`, until the transfer is successful and is confirmed by the `receiving node`.
...
During the _broadcast phase_, a `core node` that received a `transaction` from `user client` will broadcast it to other `core nodes`. 

==== Pending queue phase

During the _pending queue phase_, a `transaction` received from `user client` or from `transaction broadcast` is validated and placed into the`pending queue`.
During the validation, `transaction signature` and format are verified. 

Note that the `pending queue` has fixed memory capacity. If the `pending queue` is full, adding a new `transaction` to the `pending queue` will cause some `transactions` to be dropped from the `pending queue`. Ethereum-compatible blockchains, including SKALE, drop transactions with the smallest `gas price`.

==== Block proposal phase

During the _block proposal phase _each SKALE `node` builds a `block proposal`.  A `block proposal` is an ordered list of `transactions`.
...
Note, that if there are no `transactions` in the pending queue, the node will wait for `EMPTY_BLOCK_INTERVAL` and then create a empty `block proposal`.
The purpose of `EMPTY_BLOCK_INTERVAL` is for the network to show to an external observer that it is alive. Even if `users` do not submit transaction, the `blockchain` will still produce `empty blocks`. On SKALE mainnet `EMPTY_BLOCK_INTERVAL` is set to 10 seconds.
Note that, empty blocks take storage space, so one could set `EMPTY_BLOCK_INTERVAL` to even larger value to save space.
...
Once a `receiving node` receives a `block proposal`, it will sign a `Data Availability Signature` and pass it to the `sending node`.  The `receiving node` will only sign a single unique proposal for a given `proposer node` and `block_id`. In this way, the network enforces `block proposal` uniqueness.

Once the `sending node` collects `DA signatures` from `2/3 N` of nodes, it will merge the signatures into a `DA proof`. The `DA proof` proves that the proposal has been widely distributed over the network.
...
Second, since `DA proof` creation requires a `2/3 N` signature of nodes, the `proposal` is _guaranteed to be unique_. 
A malicious proposer is not able to create two different proposals an obtain DA proofs for both of them.


==== Block consensus phase

Once a node receives `DA proofs` from `2/3 N` of nodes, the node will start the _block consensus phase_. for each proposer.

During _block consensus phase_, the `node` will vote `1` if it received `DA proof` for a particular proposal, and vote `0` otherwise.

The nodes will then execute asynchronous binary consensus algorithm, also known as `Byzantine Generals problem`. https://en.wikipedia.org/wiki/Byzantine_fault
...
Once the `binary consensus` completes, it guarantees that all `honest node` will finish with the same value of `1` or `0'. If `honest nodes` reach `1` it is guaranteed
...
For a given `block ID`, the `N` nodes are assigned deterministic pseudo-random `priority` from `1` to `N` by taking `block ID` as the seed of pseudo-random generator. Therefore, 
if one averages over muptiple blocks, each `node` will have the same `priority`. This means that the algorithm is `fair` when averaged over multiple `blocks`.

For a given `block ID`, if `block consensus` phase outputs `1` for several proposals, the proposal with the `highest priority` is selected. Therefore, the algorithm is _deterministic_ for each `block ID`.

==== Block  signature phase

After `block consensus` decides on the `winning block`, each node will sign a `statement specifying the winning proposal` (`block signature share`) and broadcast it to other `nodes`. The node will then wait until receipt of `2/3 N` of `block signature shares` and merge the shares into `block signature`.

==== Block  finalization phase.

On completion of _block signature phase_, all `honest nodes` will possess the `block signature` but some of them may not posess the `committed block proposal` itself. 

This can happen due to a `malicious proposer`. If a `malicious proposer` attempts to attack `network liveliness` by causing the network to pause, the `malicious proposer` may intentionally withold its `proposal` from some of the  `nodes` . 

A similar situation may happen due `proposer` crashing, or due to a slow network.

Fortunately, `DA proof` requirement addresses the problem.  _It is guaranteed_, that `block proposal` that wins _block consensus phase_ has a `DA proof`. Therefore, it is widely distributed across the network.

Therefore, during _block finalization_phase_ ,if a `node` does not posess the `winning proposal`, the `node` will simply connect to other `nodes` to download the `winning proposal` and the `DA proof` for it from them. 

Note that _2/3 N_ of `nodes` are guaranteed to have a copy of the proposal after _DA proof phase_ completes, and at least one `honest node` is guaranteed to have the `DA proof`. This was the node that voted `1` for the proposal during the binary consensus phase. 

==== EVM processing phase

After _block finalization phase_ the `winning block proposal`, its `block signature` and the `DA proof` for are guaranteed to be present on each `honest node`.
These three elements for `committed block`.

The `commited block` will be then processed through `EVM` to update `EVM state`.

==== Storage phase

_Storage phase` is the last phase.`Committed block` is be stored in persistent storage. `EVM state` is updated in persistent storage to reflect `EVM state` changes that happen after `block processing`.

The `node` will then move into _block proposal phase_ for the next block.

== Protocol Deep dive 

The rest of the document specifies details of algorithms and formats used in each phase/

=== Achieving eventual delivery guarantee of messages

The `SKALE consensus` is provable under `eventual delivery guarantee` for messages. This means, that `message delivery` is guaranteed, but the `delivery` time may take arbitrarily long time. 

To provide `eventual delivery guarantee` of `messages`, the following algorithm is used:

* The `sending node` will make _multiple attempts to transfer_  `message` to the `receiving node`, until the transfer is successful and is confirmed by the `receiving node`.
...
* Each `outgoing message queue` is serviced by a separate program `thread`. The `thread` reads `messages` from the `queue` and attempts to transfer them to the `destination node`. If the `destination node` temporarily does not accept `messages`, the `thread` will periodically re-initiate transfer attempts until the `message` is delivered. The `destination node` can, therefore, temporarily go offline without causing `messages` to be lost.
...
In this document, anywhere where it is specified that a `message` is sent from `sending node` to `receivintg node`, we mean `reliable independent eventual delivery` as described above.


Note, that if a major network event (such as a global network split between the US and Europe) occurs, `SKALE consensus` may pause for a while, but when, when the network heals, consensus will self-heal by retransmissions will make the chain procede and liveliness will be restored.
...
=== Applicability of security proofs to smaller number of Byzantine Nodes

If a security proof works for a certain number of Byzantine nodes, it will work for a fewer Byzantine nodes. Indeed, an honest node can always be viewed as a Byzantine node that decided to behave honestly. Therefore, in proofs, we always assume that the system has the maximum permitted number of Byzantine nodes
...
In this specification, we assume that the `*N*` is always selected in optimal way, so that that latexmath:[N-1] is divisible by 3.
...
Note that throughout the document, for the sake of brevity, we say things like `the block proposal is received by 2/3 N of honest nodes`. Each time we say `2/3 N` in this document, we actually mean the above expression for the number of `honest nodes`.

=== Safe waiting

Throughout the protocol, if a `honest receiving node` waits for a message from each other node, it is always safe for a node to wait for `h-1` messages.
The word safe here means that the wait is always guaranteed to terminate in finite time.

This statement simply follows from the fact, that each `honest node` has `h-1` honest peers.  If the `honst node` would attempt to wait for more than `h-1` messages, the `Byzantine nodes` could attack liveliness of the protocol by simply witholding their messages.

`h-1`-wait is used by the `SKALE consensus` protocol in two places:

* an `honest node` will always wait for `h-1` `block proposals` (in addition to building and broadcasting its own `proposal`)
* an `honest node` will always wait for `h-1` `DA proofs` (in addition to broadcasting `DA proof` for its own proposal)

Note, that an 'honest node` is guaranteed to build a `DA proof` for its `block proposal`. This follows from the fact, that at least `h-1` nodes will respond with `DA signature share`, and the node will be able to generate its own `DA signature` share. `DA proof` requires `h` signature shares.

Once a node receives `h-1` DA proofs in addition to having a `DA proof` for its own proposal, the node will start a set of `N` binary consensu instances, proposing `1` if a node has `DA proof` and '0' otherwise.  Therefore, a binary string proposed by a node will typically have `2 t + 1` ones and 't' zeros.
